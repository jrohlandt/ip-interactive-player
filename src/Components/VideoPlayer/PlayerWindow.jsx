...
class PlayerWindow extends React.Component {
  ...
  render() {
    ...
    let tails = [];
    if (props.messageFromInteractor.tails) {
      tails = props.messageFromInteractor.tails;
    }
    return (
      <div className="wrapper">
        {tails.length > 0
          ? tails.map((t, index) => (
              ...
              >
                {t.label}
                <div style={{ display: "none" }}>{t.content}</div>
              </div>
            ))
          : ""}
        <div className="player-container">
          {props.muted === true ? (
            <div className={styles.mutedOverlay}>muted</div>
          ) : (
            ""
          )}

          {props.vendor === VENDORS.HTML5 &&
          props.playbackState === STATES.BUFFERING ? (
            <div className={styles.bufferingOverlay}>buffering</div>
          ) : (
            ""
          )}
          ...
          <div className="player-control-bar">
            ...
            <div
              ...
              className={
                "play-button " +
                (props.playbackState === STATES.PLAYING
                  ? "playing "
                  : "paused ")
              }
              onClick={() =>
            ...
            <div
              id="mute-button"
              className={
                "mute-button " + (props.muted === true ? "muted" : "not-muted")
              }
              onClick={() =>
                props.doAction(props.muted ? ACTIONS.UNMUTE : ACTIONS.MUTE)
              }
            >
              <img src="/icons/sound-on-icon.svg" className="sound-on" alt="" />
              <img
                src="/icons/sound-off-icon.svg"
                className="sound-off"
                alt=""
              />
            </div>
